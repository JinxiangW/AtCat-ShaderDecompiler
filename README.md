...
* [ ] 由于UE符号剔除很严重 并且运行时不需要符号的直接就是二进制丢失了一切结构信息 比如引擎内置的Global的cb 据AI所说没有提供编译后可逆的符号 我能想到的解决方法是提供一个分版本的引擎shader结构定义 但这个要分版本手动填写 我没那么多精力 还是靠后人的智慧看看有没有我分析漏的直接dump符号吧
...
UE 是由CUE4Parse解析并填充metadata符号到此工具即可完美还原符号反编译 ~~UE剔除过于严重并且屎山代码过于恶心已被气炸优先级已降低~~ 已在私有仓库完成
