...
## Mechanism 1.5 — texture name from texture↔sampler pairing in SPIR-V

For SSM_FromTextureAsset (the default `SamplerSource`), UE's HLSL
material translator emits the sampler argument *as the texture's own
paired sampler* — see `HLSLMaterialTranslator.cpp:6108-6111`:
...
i.e. `Material.Texture2D_<i>.Sample(Material.Texture2D_<i>Sampler, ...)`
is a single tight pair generated by one `Printf`. After
`RemoveUniformBuffersFromSource()` flattens the dot, the compiler
reflects them as paired top-level globals, the SRT generator binds the
sampler back to the Material UB resource list, and the texture lands
on whatever `t<n>` register the linker chose.

So even though the texture itself often does **not** appear in the
SRT's `ShaderResourceViewMap` (UE 5.1 binds a lot of material textures
through loose parameters for performance), we can still recover its
name: for every `OpSampledImage` instruction in the shader,

- walk the texture-load chain to its `OpVariable` and read its
  `Binding` decoration (the `t<n>` slot),
- walk the sampler-load chain similarly,
- look the sampler binding up in the SRT-resolved `Samplers[]` list,
- if the sampler resolves to `Material_<TexName>Sampler`, then the
  texture in this pair is `Material_<TexName>` by source-truth
  construction (the same C++ `Printf` line emitted both names).

That recovery is implemented in
`Source/Ruri.ShaderDecompiler/Unreal/UeMaterialTextureNameInferrer.cs`.
It runs **after** `UeShaderResourceTableSymbolizer.EnrichSymbolData`
(which assigns sampler names from SRT + Material UB layout), as a
final pass before the SPIR-V patcher injects `OpName` decorations.
Limited to `SSM_FromTextureAsset` — the other SSM modes route through
`GetMaterialSharedSampler(...)` which substitutes a *shared* sampler
into the call site, breaking the texture↔sampler name pair (the
runtime sampler argument is `View.MaterialTextureBilinearWrapedSampler`
or `Material.Wrap_WorldGroupSettings`, not the texture's own
`*Sampler`). For those modes the texture's name cannot be proven from
SPIR-V alone and stays anonymous (closed-world).

## What this means for the Oni_Valley_VFX fixtures

`M_Bamboo_tree_PS_1904.hlsl` final bindings:

```hlsl
Texture3D<uint4>  T0                          : register(t0);   // loose          -> closed world
Texture3D<float4> T1                          : register(t1);   // loose          -> closed world
Buffer<uint4>     View_SRV45                  : register(t2);   // SRT View       -> placeholder
Texture2D<float4> T3                          : register(t3);   // loose          -> closed world
Texture2D<float4> T4                          : register(t4);   // loose          -> closed world
Texture2D<float4> T5                          : register(t5);   // loose          -> closed world
Texture3D<float4> T6                          : register(t6);   // loose          -> closed world
Texture3D<float4> T7                          : register(t7);   // loose          -> closed world
Texture3D<float4> T8                          : register(t8);   // loose          -> closed world
Texture2D<float4> T9                          : register(t9);   // loose          -> closed world
Texture2D<float4> Material_Texture2D_1        : register(t10);  // Mechanism 1.5  ✓ "Bamboo base maps"
SamplerState View_Sampler0                    : register(s0);   // SRT View       -> placeholder
SamplerState View_Sampler39                   : register(s1);   // SRT View       -> placeholder
SamplerState OpaqueBasePass_Sampler43         : register(s2);   // SRT engine UB  -> placeholder
SamplerState IndirectLightingCache_Sampler3   : register(s3);   // SRT engine UB  -> placeholder
SamplerState Material_Texture2D_1Sampler      : register(s4);   // SRT Material   ✓ Mechanism 1
```

`M_Alpha_grass_PS_4000.hlsl` (SRT *does* bind some Material SRVs):

```hlsl
Texture2D<float4> Material_Texture2D_1                : register(t8);   // Mechanism 1.5 (SPV pair)
Texture2D<float4> Material_VirtualTexturePhysical_0   : register(t10);  // SRT Material -> CreateBufferStruct replay
Texture2D<float4> Material_VirtualTexturePhysical_1   : register(t11);  // SRT Material -> CreateBufferStruct replay
SamplerState     Material_Texture2D_1Sampler          : register(s3);   // SRT Material
```

`MI_Cliff_Parent_PS_3500.hlsl` (parent material with many textures):

```hlsl
Texture2D<float4> Material_Texture2D_7        : register(t12);  // Mechanism 1.5
Texture2D<float4> Material_Texture2D_8        : register(t13);  // Mechanism 1.5
Texture2D<float4> Material_Texture2D_9        : register(t14);  // Mechanism 1.5
SamplerState     Material_Texture2D_7Sampler  : register(s3);   // SRT Material
SamplerState     Material_Texture2D_8Sampler  : register(s4);   // SRT Material
SamplerState     Material_Texture2D_9Sampler  : register(s5);   // SRT Material
```

Material-side textures **are** consistently recovered when they use
the default `SSM_FromTextureAsset` sampler mode. Engine UBs and loose
params remain at their closed-world ceiling (`<UB>_SRV<i>` /
`<UB>_Sampler<i>` placeholder, or anonymous `T<n>`).
...
- **SPIR-V `OpSampledImage` pair where the sampler resolves to a
  `Material_*Sampler`** — name the texture by stripping the `Sampler`
  suffix (Mechanism 1.5; only valid for `SSM_FromTextureAsset`).
  Source-truth, anchored on UE's tight `Printf` pair emission.
